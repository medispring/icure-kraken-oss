...
open class KmehrExport(
	...
	fun makePersonBase(p: Patient, config: Config): PersonType {
		...
		return PersonType().apply {
			...
			sex = SexType().apply { cd = CDSEX().apply { s = "CD-SEX"; sv = "1.1"; value = p.gender?.name?.toSEXvalues() ?: CDSEXvalues.UNKNOWN } }
		...
		return ItemType().apply {
			...
			lifecycle = LifecycleType().apply {
				cd = CDLIFECYCLE().apply {
					s = "CD-LIFECYCLE"
					value = if ((FuzzyValues.getDateTime(svc.closingDate?.takeIf { it > 0 } ?: 99991231) < LocalDateTime.now())) {
	...
	open fun createItemWithContent(he: HealthElement, idx: Int, cdItem: String, contents: List<ContentType>, localIdName: String = "iCure-Service"): ItemType? {
		return ItemType().apply {
			...
			lifecycle = LifecycleType().apply {
				cd = CDLIFECYCLE().apply {
					s = "CD-LIFECYCLE"
					value = if ((!ServiceStatus.isActive(he.status)) || (FuzzyValues.getDateTime(he.closingDate?.takeIf { it > 0 } ?: 99991231) < LocalDateTime.now()))
						CDLIFECYCLEvalues.INACTIVE
					else
