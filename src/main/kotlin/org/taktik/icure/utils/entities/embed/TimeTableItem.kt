...
fun TimeTableItem.iterator(startDateAndTime: Long, endDateAndTime: Long, duration: Duration) = object : Iterator<Long> {
	...
	val daysIterator = object : Iterator<LocalDateTime> {
		var day = startLdt
		val rrit = rrule?.let { RecurrenceRule(it).iterator(FuzzyValues.getDateTime(occurenceStartDate ?: startDateAndTime).atOffset(ZoneOffset.UTC).toInstant().toEpochMilli(), TimeZone.getTimeZone("UTC")) }
